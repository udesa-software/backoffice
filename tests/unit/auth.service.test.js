...
jest.mock('../../src/config/env', () => ({
  env: { ADMIN_JWT_SECRET: 'test-admin-secret', ACCESS_TOKEN_EXPIRES_IN: '15m', REFRESH_TOKEN_EXPIRES_IN: '7d' },
}));
...
describe('authService.logout', () => {
  ...
  it('incrementa el token_version para invalidar el access token inmediatamente', async () => {
    await authService.logout('admin-uuid-1', 'some-refresh-token');
  ...
  it('devuelve un mensaje de confirmación', async () => {
    const result = await authService.logout('admin-uuid-1', 'some-refresh-token');
...
describe('authService.changePassword', () => {
  ...
  it('lanza error 404 si el admin no existe', async () => {
    ...
    await expect(authService.changePassword('admin-uuid-1', INPUT_VALIDO))
