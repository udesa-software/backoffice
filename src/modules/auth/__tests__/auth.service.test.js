...
describe('authService.login', () => {
  ...
  it('devuelve token y datos del admin cuando las credenciales son correctas', async () => {
    const result = await authService.login({ email: 'admin@udesa.edu.ar', password: 'Password1' });
  ...
  it('normaliza el email a minúsculas antes de buscar', async () => {
    await authService.login({ email: 'ADMIN@UDESA.EDU.AR', password: 'Password1' });
  ...
  it('lanza error 401 genérico si el admin no existe', async () => {
    ...
    await expect(
      authService.login({ email: 'noexiste@udesa.edu.ar', password: 'Password1' })
  ...
  it('lanza error 423 si la cuenta está bloqueada por intentos fallidos', async () => {
    ...
    await expect(
      authService.login({ email: 'admin@udesa.edu.ar', password: 'Password1' })
  ...
  it('lanza error 401 genérico si la contraseña es incorrecta', async () => {
    ...
    await expect(
      authService.login({ email: 'admin@udesa.edu.ar', password: 'wrong' })
    ).rejects.toMatchObject({ statusCode: 401, message: 'Credenciales inválidas' });
  });

  it('incrementa intentos fallidos si la contraseña es incorrecta', async () => {
    bcrypt.compare.mockResolvedValue(false);

    await expect(authService.login({ email: 'admin@udesa.edu.ar', password: 'wrong' })).rejects.toThrow();
  ...
  it('lanza error 403 si la contraseña temporal expiró', async () => {
    const expirada = new Date(Date.now() - 60 * 1000); // hace 1 minuto
  ...
  it('permite login si must_change_password es true pero la contraseña NO expiró', async () => {
    const vigente = new Date(Date.now() + 60 * 60 * 1000); // en 1 hora
    adminRepository.findByEmail.mockResolvedValue({
      ...ADMIN_DB,
      must_change_password: true,
...
describe('authService.logout', () => {
  ...
  it('incrementa el token_version para revocar todas las sesiones', async () => {
    ...
    await authService.logout('admin-uuid-1');
  ...
  it('devuelve un mensaje de confirmación', async () => {
    ...
    const result = await authService.logout('admin-uuid-1');
...
describe('authService.changePassword', () => {
  ...
  it('actualiza la contraseña y limpia must_change_password', async () => {
    await authService.changePassword('admin-uuid-1', INPUT_VALIDO);
  ...
  it('no falla si el envío del email de notificación falla', async () => {
    ...
    await expect(authService.changePassword('admin-uuid-1', INPUT_VALIDO)).resolves.toBeDefined();
  });

  it('devuelve un mensaje de éxito', async () => {
    const result = await authService.changePassword('admin-uuid-1', INPUT_VALIDO);
  ...
  it('lanza error 404 si el admin no existe', async () => {
    ...
    await expect(authService.changePassword('admin-uuid-1', INPUT_VALIDO)).rejects.toMatchObject({ statusCode: 404 });
  ...
  it('lanza error 401 si la contraseña actual es incorrecta', async () => {
    ...
    await expect(authService.changePassword('admin-uuid-1', INPUT_VALIDO)).rejects.toMatchObject({ statusCode: 401 });
    expect(adminRepository.updatePassword).not.toHaveBeenCalled();
  });
  ...
  it('lanza error 400 si la nueva contraseña es igual a la actual', async () => {
    ...
    await expect(authService.changePassword('admin-uuid-1', INPUT_VALIDO)).rejects.toMatchObject({ statusCode: 400 });
    expect(adminRepository.updatePassword).not.toHaveBeenCalled();
  });
});
